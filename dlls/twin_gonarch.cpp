...
// AI Nodes for Twin Gonarch
class CInfoTG : public CPointEntity
{
public:
	void Spawn() override;
...
LINK_ENTITY_TO_CLASS(info_twingonarch, CInfoTG);

TYPEDESCRIPTION CInfoTG::m_SaveData[] =
{
	DEFINE_FIELD(CInfoTG, m_preSequence, FIELD_STRING),
};

IMPLEMENT_SAVERESTORE(CInfoTG, CPointEntity);
...
bool CInfoTG::KeyValue(KeyValueData* pkvd)
{
...
// Mortar shot entity
//=========================================================
class CTMortar : public CBaseEntity
{
public:
	void Spawn() override;

	static CTMortar* Shoot(edict_t* pOwner, Vector vecStart, Vector vecVelocity);
...
LINK_ENTITY_TO_CLASS(tmortar, CTMortar);

TYPEDESCRIPTION CTMortar::m_SaveData[] =
	{
		DEFINE_FIELD(CTMortar, m_maxFrame, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CTMortar, CBaseEntity);
...
int gTGSpitSprite, gTGSpitDebrisSprite;
Vector VecCheckSplatToss2(entvars_t* pev, const Vector& vecSpot1, Vector vecSpot2, float maxHeight);
void TGMortarSpray(const Vector& position, const Vector& direction, int spriteModel, int count);


// UNDONE:
//
#define BIG_CHILDCLASS "monster_babycrab"
#define	SF_BIGMOMMA_PHASE2	64

class CTwinGonarch : public CBaseMonster
{
public:
	void Spawn() override;
	...
	{
		CInfoTG* pTarget = (CInfoTG*)(CBaseEntity*)m_hTargetEnt;
		if (pTarget)
		{
...
LINK_ENTITY_TO_CLASS(monster_twin_gonarch, CTwinGonarch);

TYPEDESCRIPTION CTwinGonarch::m_SaveData[] =
	{
		DEFINE_FIELD(CTwinGonarch, m_nodeTime, FIELD_TIME),
		DEFINE_FIELD(CTwinGonarch, m_crabTime, FIELD_TIME),
		DEFINE_FIELD(CTwinGonarch, m_mortarTime, FIELD_TIME),
		DEFINE_FIELD(CTwinGonarch, m_painSoundTime, FIELD_TIME),
		DEFINE_FIELD(CTwinGonarch, m_crabCount, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CTwinGonarch, CBaseMonster);

const char* CTwinGonarch::pChildDieSounds[] =
	{
		"tgonarch/tgon_childdie1.wav",
		"tgonarch/tgon_childdie2.wav",
		"tgonarch/tgon_childdie3.wav",
};

const char* CTwinGonarch::pSackSounds[] =
	{
		"tgonarch/tgon_sack1.wav",
		"tgonarch/tgon_sack2.wav",
		"tgonarch/tgon_sack3.wav",
};

const char* CTwinGonarch::pDeathSounds[] =
	{
		"tgonarch/tgon_die1.wav",
};

const char* CTwinGonarch::pAttackSounds[] =
	{
		"tgonarch/tgon_attack1.wav",
		"tgonarch/tgon_attack2.wav",
		"tgonarch/tgon_attack3.wav",
};
const char* CTwinGonarch::pAttackHitSounds[] =
	{
...
const char* CTwinGonarch::pBirthSounds[] =
	{
		"tgonarch/tgon_birth1.wav",
		"tgonarch/tgon_birth2.wav",
		"tgonarch/tgon_birth3.wav",
};

const char* CTwinGonarch::pAlertSounds[] =
	{
		"tgonarch/tgon_alert1.wav",
		"tgonarch/tgon_alert2.wav",
		"tgonarch/tgon_alert3.wav",
};

const char* CTwinGonarch::pPainSounds[] =
	{
		"tgonarch/tgon_pain2.wav",
		"tgonarch/tgon_pain4.wav",
		"tgonarch/tgon_pain5.wav",
};

const char* CTwinGonarch::pFootSounds[] =
	{
		"tgonarch/tgon_step1.wav",
		"tgonarch/tgon_step2.wav",
		"tgonarch/tgon_step3.wav",
};



bool CTwinGonarch::KeyValue(KeyValueData* pkvd)
{
#if 0
...
//=========================================================
void CTwinGonarch::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	switch (pEvent->event)
	{
	...
	case BIG_AE_MELEE_ATTACK1:
	{
		...
		if (pHurt)
		{
			pHurt->TakeDamage(pev, pev, gSkillData.twinGonarchDmgSlash, DMG_CRUSH | DMG_SLASH);
	...
	case BIG_AE_STEP3: // Footstep back left
		...
		{
		case 0:
			EMIT_SOUND_DYN(ENT(pev), CHAN_ITEM, "tgonarch/tgon_step1.wav", 1.0, ATTN_NORM, 0, RANDOM_LONG(95, 105));
			break;
		case 1:
			EMIT_SOUND_DYN(ENT(pev), CHAN_ITEM, "tgonarch/tgon_step2.wav", 1.0, ATTN_NORM, 0, RANDOM_LONG(95, 105));
			break;		
		case 2:
			EMIT_SOUND_DYN(ENT(pev), CHAN_ITEM, "tgonarch/tgon_step3.wav", 1.0, ATTN_NORM, 0, RANDOM_LONG(95, 105));
			break;
		}
		break;
...
void CTwinGonarch::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	if (ptr->iHitgroup != 1)
	{
		// didn't hit the sack?
...
bool CTwinGonarch::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
...
void CTwinGonarch::DeathNotice(entvars_t* pevChild)
{
...
void CTwinGonarch::LaunchMortar()
{
	if (pev->spawnflags & SF_BIGMOMMA_PHASE2)
		m_mortarTime = gpGlobals->time + RANDOM_FLOAT(2, 4);
	else
		m_mortarTime = gpGlobals->time + RANDOM_FLOAT(5, 15);
	...
	vecSpitOffset = (gpGlobals->v_right * 0 + gpGlobals->v_forward * 0 + gpGlobals->v_up * 16);
	vecSpitOffset = (pev->origin + vecSpitOffset);
	vecSpitDir = ((m_hEnemy->pev->origin + m_hEnemy->pev->view_ofs) - vecSpitOffset).Normalize();

	CTMortar* pBomb = CTMortar::Shoot(edict(), vecSpitOffset, vecSpitDir * 2000);
	...
	if (pev->spawnflags & SF_BIGMOMMA_PHASE2)
	{
		...
		CTMortar* pBomb2 = CTMortar::Shoot(edict(), startPos, pev->movedir);
		pBomb2->pev->gravity = 1.0;
	}

	TGMortarSpray(vecSpitOffset, Vector(0, 0, 1), gTGSpitSprite, 24);
}

//=========================================================
// Spawn
//=========================================================
void CTwinGonarch::Spawn()
{
	Precache();

	SET_MODEL(ENT(pev), "models/twin_gonarch.mdl");
	UTIL_SetSize(pev, Vector(-32, -32, 0), Vector(32, 32, 64));

	pev->solid = SOLID_SLIDEBOX;
	pev->movetype = MOVETYPE_STEP;
	m_bloodColor = BLOOD_COLOR_GREEN;
	pev->health = 150 * gSkillData.twinGonarchHealthFactor;
...
//=========================================================
void CTwinGonarch::Precache()
{
	PRECACHE_MODEL("models/twin_gonarch.mdl");
	...
	gTGSpitSprite = PRECACHE_MODEL("sprites/mommaspout.spr"); // client side spittle.
	gTGSpitDebrisSprite = PRECACHE_MODEL("sprites/mommablob.spr");
...
void CTwinGonarch::NodeStart(int iszNextNode)
{
...
void CTwinGonarch::NodeReach()
{
	...
	if (0 != pTarget->pev->health)
		pev->max_health = pev->health = pTarget->pev->health * gSkillData.twinGonarchHealthFactor;
...
// Slash
bool CTwinGonarch::CheckMeleeAttack1(float flDot, float flDist)
{
	if (flDot >= 0.7)
	{
...
// Lay a crab
bool CTwinGonarch::CheckMeleeAttack2(float flDot, float flDist)
{
	return CanLayCrab();
}
...
// Mortar launch
bool CTwinGonarch::CheckRangeAttack1(float flDot, float flDist)
{
	if (flDist <= BIG_MORTARDIST && m_mortarTime < gpGlobals->time)
	{
		...
		if (pEnemy)
		{
			Vector startPos = pev->origin;
			startPos.z += 180;
			pev->movedir = VecCheckSplatToss2(pev, startPos, pEnemy->BodyTarget(pev->origin), RANDOM_FLOAT(150, 500));
...
Schedule_t slTGBigNode[] =
	{
		{tlTGBigNode,
			ARRAYSIZE(tlTGBigNode),
			0,
			0,
			"Big Node"},
};
...
Schedule_t slTGNodeFail[] =
	{
		{tlTGNodeFail,
			ARRAYSIZE(tlTGNodeFail),
			0,
			0,
			"NodeFail"},
};

DEFINE_CUSTOM_SCHEDULES(CTwinGonarch){
	slTGBigNode,
	slTGNodeFail,
};

IMPLEMENT_CUSTOM_SCHEDULES(CTwinGonarch, CBaseMonster);




Schedule_t* CTwinGonarch::GetScheduleOfType(int Type)
{
	switch (Type)
	{
	case SCHED_BIG_NODE:
...
bool CTwinGonarch::ShouldGoToNode()
{
...
Schedule_t* CTwinGonarch::GetSchedule()
{
	if (ShouldGoToNode())
	{
...
void CTwinGonarch::StartTask(Task_t* pTask)
{
	switch (pTask->iTask)
	{
	case TASK_FIND_NODE:
...
// RunTask
//=========================================================
void CTwinGonarch::RunTask(Task_t* pTask)
{
	switch (pTask->iTask)
	{
	case TASK_MOVE_TO_NODE_RANGE:
...
void CTMortar::Spawn()
{
	...
	pev->classname = MAKE_STRING("tmortar");
...
CTMortar* CTMortar::Shoot(edict_t* pOwner, Vector vecStart, Vector vecVelocity)
{
	CTMortar* pSpit = GetClassPtr((CTMortar*)NULL);
	pSpit->Spawn();
	...
	pSpit->pev->scale = 2.5;
	pSpit->SetThink(&CTMortar::Animate);
...
void CTMortar::Touch(CBaseEntity* pOther)
{
	TraceResult tr;
	int iPitch;
	...
	RadiusDamage(pev->origin, pev, pevOwner, gSkillData.twinGonarchDmgBlast, gSkillData.twinGonarchRadiusBlast, CLASS_NONE, DMG_ACID);
	UTIL_Remove(this);
}
