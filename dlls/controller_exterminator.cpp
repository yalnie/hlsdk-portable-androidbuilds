...
//=========================================================
#define CONTROLLER_EX_AE_HEAD_OPEN 1
#define CONTROLLER_EX_AE_BALL_SHOOT 2
#define CONTROLLER_EX_AE_SMALL_SHOOT 3
#define CONTROLLER_EX_AE_POWERUP_FULL 4
#define CONTROLLER_EX_AE_POWERUP_HALF 5

#define CONTROLLER_EX_FLINCH_DELAY 2 // at most one flinch every n secs

class CControllerExterminator : public CSquadMonster
{
public:
...
LINK_ENTITY_TO_CLASS(monster_controller_exterminator, CControllerExterminator);

TYPEDESCRIPTION CControllerExterminator::m_SaveData[] =
{
	DEFINE_ARRAY(CControllerExterminator, m_pBall, FIELD_CLASSPTR, 2),
	DEFINE_ARRAY(CControllerExterminator, m_iBall, FIELD_INTEGER, 2),
	DEFINE_ARRAY(CControllerExterminator, m_iBallTime, FIELD_TIME, 2),
	DEFINE_ARRAY(CControllerExterminator, m_iBallCurrent, FIELD_INTEGER, 2),
	DEFINE_FIELD(CControllerExterminator, m_vecEstVelocity, FIELD_VECTOR),
};
IMPLEMENT_SAVERESTORE(CControllerExterminator, CSquadMonster);


const char* CControllerExterminator::pAttackSounds[] =
{
...
const char* CControllerExterminator::pIdleSounds[] =
{
...
const char* CControllerExterminator::pAlertSounds[] =
{
...
const char* CControllerExterminator::pPainSounds[] =
{
...
const char* CControllerExterminator::pDeathSounds[] =
{
...
// relationship table.
//=========================================================
int CControllerExterminator::Classify()
{
	return CLASS_ALIEN_SLAVE_EMPOWERED;
}

int CControllerExterminator::IRelationship(CBaseEntity* pTarget)
{
	// Don't hate on the explosive houndeyes, yet.
	if (FClassnameIs(pTarget->pev, "monster_houndeye_explosive"))
	{
		return R_AL;
	}
...
//=========================================================
void CControllerExterminator::SetYawSpeed()
{
	int ys;
...
bool CControllerExterminator::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
...
void CControllerExterminator::Killed(entvars_t* pevAttacker, int iGib)
{
	// shut off balls
	/*
	m_iBall[0] = 0;
...
void CControllerExterminator::GibMonster()
{
	// delete balls
	if (m_pBall[0])
	{
...
void CControllerExterminator::PainSound()
{
	if (RANDOM_LONG(0, 5) < 2)
		EMIT_SOUND_ARRAY_DYN(CHAN_VOICE, pPainSounds);
}

void CControllerExterminator::AlertSound()
{
	EMIT_SOUND_ARRAY_DYN(CHAN_VOICE, pAlertSounds);
}

void CControllerExterminator::IdleSound()
{
	EMIT_SOUND_ARRAY_DYN(CHAN_VOICE, pIdleSounds);
}

void CControllerExterminator::AttackSound()
{
	EMIT_SOUND_ARRAY_DYN(CHAN_VOICE, pAttackSounds);
}

void CControllerExterminator::DeathSound()
{
...
//=========================================================
void CControllerExterminator::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	switch (pEvent->event)
	{
	case CONTROLLER_EX_AE_HEAD_OPEN:
	{
		pev->framerate = 2;
	...
	case CONTROLLER_EX_AE_BALL_SHOOT:
	{
		pev->framerate = 2;
		...
		CBaseMonster* pBall = (CBaseMonster*)Create("controller_ex_head_ball", vecStart, pev->angles, edict());
	...
	case CONTROLLER_EX_AE_SMALL_SHOOT:
	{
		pev->framerate = 2;
		AttackSound();
	...
	break;
	case CONTROLLER_EX_AE_POWERUP_FULL:
	{
		pev->framerate = 2;
		m_iBall[0] = 255;
		m_iBallTime[0] = gpGlobals->time + atoi(pEvent->options) / 15.0;
	...
	break;
	case CONTROLLER_EX_AE_POWERUP_HALF:
	{
		pev->framerate = 2;
		m_iBall[0] = 192;
		m_iBallTime[0] = gpGlobals->time + atoi(pEvent->options) / 15.0;
...
// Spawn
//=========================================================
void CControllerExterminator::Spawn()
{
	Precache();

	SET_MODEL(ENT(pev), "models/controller_ext.mdl");
	...
	pev->health = gSkillData.controllerExtHealth;
...
//=========================================================
void CControllerExterminator::Precache()
{
	PRECACHE_MODEL("models/controller_ext.mdl");
	...
	UTIL_PrecacheOther("controller_ex_energy_ball");
	UTIL_PrecacheOther("controller_ex_head_ball");
}
...
Task_t tlControllerExChaseEnemy[] =
{
...
Schedule_t slControllerExChaseEnemy[] =
{
	{tlControllerExChaseEnemy,
		ARRAYSIZE(tlControllerExChaseEnemy),
		bits_COND_NEW_ENEMY |
...
Schedule_t slControllerExStrafe[] =
{
	{tlControllerExStrafe,
		ARRAYSIZE(tlControllerExStrafe),
		bits_COND_NEW_ENEMY,
		0,
		"ControllerStrafe"},
};
...
Task_t tlControllerExTakeCover[] =
{
	{TASK_WAIT, (float)0.2},
...
Schedule_t slControllerExTakeCover[] =
{
	{tlControllerExTakeCover,
		ARRAYSIZE(tlControllerExTakeCover),
		bits_COND_NEW_ENEMY,
		0,
		"ControllerTakeCover"},
};
...
Schedule_t slControllerExFail[] =
{
	{tlControllerExFail,
		ARRAYSIZE(tlControllerExFail),
		0,
		0,
		"ControllerFail"},
};
...
DEFINE_CUSTOM_SCHEDULES(CControllerExterminator) {
	slControllerExChaseEnemy,
		slControllerExStrafe,
		slControllerExTakeCover,
		slControllerExFail,
};

IMPLEMENT_CUSTOM_SCHEDULES(CControllerExterminator, CSquadMonster);
...
// StartTask
//=========================================================
void CControllerExterminator::StartTask(Task_t* pTask)
{
	switch (pTask->iTask)
	{
...
Vector IntersectEx(Vector vecSrc, Vector vecDst, Vector vecMove, float flSpeed)
{
	...
	// ALERT( at_console, "IntersectEx %f\n", t );
...
int CControllerExterminator::LookupFloat()
{
...
// RunTask
//=========================================================
void CControllerExterminator::RunTask(Task_t* pTask)
{
	...
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			...
			if (m_hEnemy != NULL)
			{
				...
				vecDir = IntersectEx(vecSrc, m_hEnemy->BodyTarget(pev->origin), m_vecEstVelocity, gSkillData.controllerExtSpeedBall);
				float delta = 0.03490; // +-2 degree
				vecDir = vecDir + Vector(RANDOM_FLOAT(-delta, delta), RANDOM_FLOAT(-delta, delta), RANDOM_FLOAT(-delta, delta)) * gSkillData.controllerExtSpeedBall;

				vecSrc = vecSrc + vecDir * (gpGlobals->time - m_flShootTime);
				CBaseMonster* pBall = (CBaseMonster*)Create("controller_ex_energy_ball", vecSrc, pev->angles, edict());
...
// of the proper type.
//=========================================================
Schedule_t* CControllerExterminator::GetSchedule()
{
	...
	{
		Vector vecTmp = IntersectEx(Vector(0, 0, 0), Vector(100, 4, 7), Vector(2, 10, -3), 20.0);
...
//=========================================================
//=========================================================
Schedule_t* CControllerExterminator::GetScheduleOfType(int Type)
{
	// ALERT( at_console, "%d\n", m_iFrustration );
	switch (Type)
	{
	case SCHED_CHASE_ENEMY:
		return slControllerExChaseEnemy;
	case SCHED_RANGE_ATTACK1:
		return slControllerExStrafe;
	case SCHED_RANGE_ATTACK2:
	case SCHED_MELEE_ATTACK1:
	case SCHED_MELEE_ATTACK2:
	case SCHED_TAKE_COVER_FROM_ENEMY:
		return slControllerExTakeCover;
	case SCHED_FAIL:
		return slControllerExFail;
	}
...
//
//=========================================================
bool CControllerExterminator::CheckRangeAttack1(float flDot, float flDist)
{
...
bool CControllerExterminator::CheckRangeAttack2(float flDot, float flDist)
{
...
bool CControllerExterminator::CheckMeleeAttack1(float flDot, float flDist)
{
	return false;
}


void CControllerExterminator::SetActivity(Activity NewActivity)
{
...
// RunAI
//=========================================================
void CControllerExterminator::RunAI()
{
	CBaseMonster::RunAI();
	Vector vecStart, angleGun;
...
void CControllerExterminator::Stop()
{
...
void CControllerExterminator::Move(float flInterval)
{
...
bool CControllerExterminator::ShouldAdvanceRoute(float flWaypointDist)
{
...
int CControllerExterminator::CheckLocalMove(const Vector& vecStart, const Vector& vecEnd, CBaseEntity* pTarget, float* pflDist)
{
	TraceResult tr;
...
void CControllerExterminator::MoveExecute(CBaseEntity* pTargetEnt, const Vector& vecDir, float flInterval)
{
...
//=========================================================
class CControllerExHeadBall : public CBaseMonster
{
	void Spawn() override;
	void Precache() override;
...
LINK_ENTITY_TO_CLASS(controller_ex_head_ball, CControllerExHeadBall);



void CControllerExHeadBall::Spawn()
{
	Precache();
	// motor
	pev->movetype = MOVETYPE_FLY;
	...
	SetThink(&CControllerExHeadBall::HuntThink);
	SetTouch(&CControllerExHeadBall::BounceTouch);
...
void CControllerExHeadBall::Precache()
{
...
void CControllerExHeadBall::HuntThink()
{
	pev->nextthink = gpGlobals->time + 0.1;
	...
	if ((m_hEnemy->Center() - pev->origin).Length() < 64)
	{
		...
		SetThink(&CControllerExHeadBall::DieThink);
...
void CControllerExHeadBall::DieThink()
{
	UTIL_Remove(this);
}


void CControllerExHeadBall::MovetoTarget(Vector vecTarget)
{
	// accelerate
...
void CControllerExHeadBall::Crawl()
{
...
void CControllerExHeadBall::BounceTouch(CBaseEntity* pOther)
{
...
class CControllerExZapBall : public CBaseMonster
{
	void Spawn() override;
	void Precache() override;
...
LINK_ENTITY_TO_CLASS(controller_ex_energy_ball, CControllerExZapBall);


void CControllerExZapBall::Spawn()
{
	Precache();
	// motor
	pev->movetype = MOVETYPE_FLY;
	...
	SetThink(&CControllerExZapBall::AnimateThink);
	SetTouch(&CControllerExZapBall::ExplodeTouch);
...
void CControllerExZapBall::Precache()
{
	PRECACHE_MODEL("sprites/xspark4.spr");
	// PRECACHE_SOUND("debris/zap4.wav");
	// PRECACHE_SOUND("weapons/electro4.wav");
...
void CControllerExZapBall::AnimateThink()
{
	pev->nextthink = gpGlobals->time + 0.1;
...
void CControllerExZapBall::ExplodeTouch(CBaseEntity* pOther)
{
	if (0 != pOther->pev->takedamage)
	{
		...
		ClearMultiDamage();
		pOther->TraceAttack(pevOwner, gSkillData.controllerExtDmgBall, pev->velocity.Normalize(), &tr, DMG_ENERGYBEAM);
