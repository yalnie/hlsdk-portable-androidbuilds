...
int CPenguinGrenade::Classify()
{
	...
	if (m_hEnemy != NULL)
	{
		...
		case CLASS_ALIEN_PREDATOR:
			m_iMyClass = 0;
			return CLASS_HUMAN_MILITARY; // aliens gotta hate them too
		}
		m_iMyClass = 0;
	}
...
void CPenguinGrenade::Killed(entvars_t* pevAttacker, int iGib)
{
	...
	// since penguin grenades never leave a body behind, clear out their takedamage now.
...
bool CPenguin::Deploy()
{
	...
	const bool result = DefaultDeploy("models/v_penguin.mdl", "models/p_penguin.mdl", PENGUIN_UP, "squeak");
