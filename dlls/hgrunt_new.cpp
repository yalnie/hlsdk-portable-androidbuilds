...
#define SF_HUMAN_GRUNT_IS_DEAF		8

#define HGRUNT_CLIP_SIZE 36	 // how many bullets in a clip? - NOTE: 3 round burst sound, so keep as 3 * x!
#define HGRUNT_VOL 0.35		 // volume of grunt sounds
#define HGRUNT_ATTN ATTN_NORM // attenutation of grunt sentences
...
#define HGRUNT_HEAD_GROUP 1
#define HGRUNT_HEAD_MASK 0
#define HGRUNT_HEAD_HELMET 1
#define HGRUNT_HEAD_COMMANDER 2
#define HGRUNT_HEAD_SHOTGUNNER 3
#define HGRUNT_HEAD_GRENADIER 4

#define HGRUNT_GUN_GROUP 2
#define HGRUNT_GUN_MP5 0
#define HGRUNT_GUN_SHOTGUN 1
#define HGRUNT_GUN_SNIPER 2
#define HGRUNT_GUN_NONE 3
...
void CHGrunt::SpeakSentence()
{
	...
	if (FOkToSpeak())
	{
		SENTENCEG_PlayRndSz(ENT(pev), pGruntSentences[m_iSentence], HGRUNT_SENTENCE_VOLUME, HGRUNT_ATTN, 0, m_voicePitch);
		JustSpoke();
	}
}
...
void CHGrunt::GibMonster()
{
	...
	if (GetBodygroup(HGRUNT_GUN_GROUP) != HGRUNT_GUN_NONE)
	{ // throw a gun if the grunt has one
		...
		else if (FBitSet(pev->weapons, HGRUNT_SNIPERRIFLE))
		{
...
void CHGrunt::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HGRUNT_AE_DROP_GUN:
	{
		if (GetBodygroup(HGRUNT_GUN_GROUP) != HGRUNT_GUN_NONE)
		{
			...
			SetBodygroup(HGRUNT_GUN_GROUP, HGRUNT_GUN_NONE);
	...
	case HGRUNT_AE_CAUGHT_ENEMY:
	{
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(ENT(pev), "HG_ALERT", HGRUNT_SENTENCE_VOLUME, HGRUNT_ATTN, 0, m_voicePitch);
			JustSpoke();
		}
	}
	break;
...
void CHGrunt::Spawn()
{
	...
	SetBodygroup(HGRUNT_HEAD_GROUP, RANDOM_LONG(0, 1)); // select the mask or the helmet
	...
	if (FBitSet(pev->weapons, HGRUNT_SHOTGUN))
	{
		SetBodygroup(HGRUNT_GUN_GROUP, HGRUNT_GUN_SHOTGUN);
		m_cClipSize = 8;
	}	
	else if (FBitSet(pev->weapons, HGRUNT_SNIPERRIFLE))
	{
		SetBodygroup(HGRUNT_GUN_GROUP, HGRUNT_GUN_SNIPER);
		m_cClipSize = 1;
	}
	else
	{
		m_cClipSize = HGRUNT_CLIP_SIZE;
	}
	m_cAmmoLoaded = m_cClipSize;
	...
	if (FBitSet(pev->weapons, HGRUNT_SHOTGUN))
	{
		SetBodygroup(HGRUNT_HEAD_GROUP, HGRUNT_HEAD_SHOTGUNNER);
	}
	else if (FBitSet(pev->weapons, HGRUNT_GRENADELAUNCHER))
	{
		SetBodygroup(HGRUNT_HEAD_GROUP, HGRUNT_HEAD_GRENADIER);
...
void CHGrunt::DeathSound()
{
	...
	if ((pev->spawnflags & SF_HUMAN_GRUNT_IS_DEAF) == 0)
	{
...
Task_t tlGruntInvestigateSound[] =
{
	{TASK_STOP_MOVING, (float)0},
...
Schedule_t slGruntInvestigateSound[] =
{
	{tlGruntInvestigateSound,
		ARRAYSIZE(tlGruntInvestigateSound),
		bits_COND_NEW_ENEMY |
			bits_COND_SEE_FEAR |
			bits_COND_LIGHT_DAMAGE |
			bits_COND_HEAVY_DAMAGE |
...
Schedule_t* CHGrunt::GetSchedule()
{
	...
	if (HasConditions(bits_COND_HEAR_SOUND))
	{
		...
		if (pSound)
		{
			if ((pSound->m_iType & bits_SOUND_DANGER) != 0)
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(ENT(pev), "HG_GREN", HGRUNT_SENTENCE_VOLUME, HGRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
	...
	case MONSTERSTATE_ALERT:
	{
		...
		if (HasConditions(bits_COND_HEAR_SOUND) && (m_hEnemy == NULL)
			&& (pev->spawnflags & SF_HUMAN_GRUNT_IS_DEAF) == 0)
		{
			CSound* pSound;
			pSound = PBestSound();
			...
			{
				return GetScheduleOfType(SCHED_GRUNT_INVESTIGATE_SOUND);
			}
		}
	}
	break;
	case MONSTERSTATE_COMBAT:
	{
				...
				else
				{
					...
					if (FOkToSpeak()) // && RANDOM_LONG(0,1))
					{
						if ((m_hEnemy != NULL) && m_hEnemy->IsPlayer())
							...
							// monster
							SENTENCEG_PlayRndSz(ENT(pev), "HG_MONST", HGRUNT_SENTENCE_VOLUME, HGRUNT_ATTN, 0, m_voicePitch);
		...
		else if (HasConditions(bits_COND_ENEMY_OCCLUDED))
		{
			if (HasConditions(bits_COND_CAN_RANGE_ATTACK2) && OccupySlot(bits_SLOTS_HGRUNT_GRENADE))
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(ENT(pev), "HG_THROW", HGRUNT_SENTENCE_VOLUME, HGRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
				return GetScheduleOfType(SCHED_RANGE_ATTACK2);
			}
			...
			else
			{
				...
				if (FOkToSpeak() && RANDOM_LONG(0, 1))
				{
					SENTENCEG_PlayRndSz(ENT(pev), "HG_TAUNT", HGRUNT_SENTENCE_VOLUME, HGRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
...
Schedule_t* CHGrunt::GetScheduleOfType(int Type)
{
	...
	case SCHED_GRUNT_INVESTIGATE_SOUND:
	{
		return &slGruntInvestigateSound[0];
	}
...
void CDeadHGrunt::Spawn()
{
	...
	{
	case 0: // Mask
		SetBodygroup(HGRUNT_HEAD_GROUP, HGRUNT_HEAD_MASK);
		break;
	case 1: // Helmet
		SetBodygroup(HGRUNT_HEAD_GROUP, HGRUNT_HEAD_HELMET);
		break;
	case 2: // Commander
		SetBodygroup(HGRUNT_HEAD_GROUP, HGRUNT_HEAD_COMMANDER);
		break;
	case 3: // Spec Ops Mask
		SetBodygroup(HGRUNT_HEAD_GROUP, HGRUNT_HEAD_SHOTGUNNER);
		break;
	case 4: // Grenadier
		SetBodygroup(HGRUNT_HEAD_GROUP, HGRUNT_HEAD_GRENADIER);
...
//=========================================================
// DEAD HGRUNT PROP (MEDIC)
//=========================================================
class CDeadMedic : public CBaseMonster
{
public:
	void Spawn() override;
	int Classify() override { return CLASS_HUMAN_MILITARY; }
...
bool CDeadMedic::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "pose"))
	{
		m_iPose = atoi(pkvd->szValue);
...
LINK_ENTITY_TO_CLASS(monster_medic_dead, CDeadMedic);
...
{
	PRECACHE_MODEL("models/dead_medic.mdl");
	SET_MODEL(ENT(pev), "models/dead_medic.mdl");
	...
	if (pev->sequence == -1)
	{
		ALERT(at_console, "Dead hgrunt medic with bad pose\n");
	}
